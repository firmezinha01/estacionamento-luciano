...
//     const escposBase64 = await res.text();

//     // 2. Conectar ao QZ Tray
//     await qz.websocket.connect();

//     // 3. Encontrar impressora (você pode trocar o nome depois)
//     const printer = await qz.printers.find("SP");

//     const config = qz.configs.create(printer);

//     // 4. Enviar ESC/POS como BASE64
//     const data = [
//       { type: 'raw', format: 'base64', data: escposBase64 }
//     ];
...
// =======================
// IMPRESSÃO VIA HTML (PC)
// =======================
function imprimirTicketHTML() {
  window.print();
}

// Preencher ticket antes de imprimir
function preencherTicket(t, calculo, saida) {
  document.getElementById("p_id").textContent = t.id;
  document.getElementById("p_name").textContent = t.nome;
  document.getElementById("p_tel").textContent = t.telefone;
  document.getElementById("p_marca_modelo").textContent = t.marca_modelo;
  document.getElementById("p_plate").textContent = t.placa;
  document.getElementById("p_slot").textContent = t.vaga || "--";

  const entrada = parseLocalTimestamp(t.entrada);
  document.getElementById("p_start").textContent = dtDisp(entrada);
  document.getElementById("p_end").textContent = dtDisp(saida);

  const diffMs = saida - entrada;
  const diffMin = Math.floor(diffMs / 60000);
  const horas = Math.floor(diffMin / 60);
  const minutos = diffMin % 60;
  document.getElementById("p_duration").textContent = `${horas}h ${minutos}m`;

  document.getElementById("p_dailyValue").textContent = fmtBRL(t.valor_diario);
  document.getElementById("p_subtotal").textContent = fmtBRL(calculo.subtotal);
  document.getElementById("p_discount").textContent = fmtBRL(calculo.desconto);
  document.getElementById("p_extra").textContent = fmtBRL(calculo.taxa_extra);
  document.getElementById("p_total").textContent = fmtBRL(calculo.total);
  document.getElementById("p_method").textContent = t.pagamento || "--";
}
...
async function createTicket() {
  ...
  if (isNaN(dailyValue) || dailyValue <= 0) {
    dailyValueField.classList.add("error");
    if (errorBox) errorBox.textContent = "Preencher campos obrigatórios";
    return;
  }
  ...
  try {
    ...
    if (ticket && ticket.id) {
      ...
      showMessage("Erro ao criar ticket", "error");
    }
...
async function finalizeTicketAndHistory(method) {
  ...
  try {
    ...
    if (isMobile) {
      ...
      preencherTicket(updated, updated, parseLocalTimestamp(updated.saida));
      imprimirTicketHTML();
    }
...
async function reimprimirTicket(id) {
  ...
  preencherTicket(ticket, ticket, parseLocalTimestamp(ticket.saida));
  imprimirTicketHTML();
}
